...
def _show_settings_help() -> None:
    showText(
        ...
        "Again uses its configured base value times the slot multiplier when "
        "its base value is not 0. Hard uses its configured base value without "
        "spinning. Good and Easy use their configured base values when a "
        "trigger check produces a spin. Any answer with a base value of 0 "
        "skips the spin."
    )
